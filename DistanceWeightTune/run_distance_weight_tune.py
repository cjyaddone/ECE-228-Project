...
SEED = 42
POS_WEIGHT_CAP = 10.0
DISTANCE_WEIGHT_BINS = [
    {"min_km": 0.0, "max_km": 5.0, "weight": 1.0},
    {"min_km": 5.0, "max_km": 30.0, "weight": 2.0},
    {"min_km": 30.0, "max_km": 100.0, "weight": 4.0},
    {"min_km": 100.0, "max_km": None, "weight": 8.0},
]
...
def distance_weight_for_step(step_km: float) -> float:
    if step_km <= 5.0:
        return 1.0
    if step_km <= 30.0:
        return 2.0
...
def multitask_loss(
    ...
    moving_sample_weights = moving_weight * sample_weights.view(-1, 1)
    ...
    unweighted_total = fly_loss + unweighted_distance_loss + 0.5 * unweighted_direction_loss
    return total, {
...
def train_triline_model(
    *,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for features, bird_ids, labels, log_distance_targets, direction_targets, distance_weights in train_loader:
            features = features.to(device)
            bird_ids = bird_ids.to(device)
            ...
            loss, components = multitask_loss(
                outputs, labels, log_distance_targets, direction_targets, distance_weights, fly_loss_fn
            )
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
        ...
        test_loss, test_unweighted_loss, labels_np, probabilities, _, _ = evaluate_triline(
            model, test_loader, fly_loss_fn, device
        )
            ...
            torch.save(
                {
                    ...
                    "distance_weight_bins": DISTANCE_WEIGHT_BINS,
                    "distance_weight_scope": "triline_distance_and_direction_losses",
                },
                checkpoint_path,
            )
        else:
    ...
    metrics_payload: dict[str, object] = {
        ...
        "distance_weight_scope": "triline_distance_and_direction_losses",
        "distance_weight_bins": DISTANCE_WEIGHT_BINS,
        "distance_weight_mean_train": float(window_data.distance_weights[train_idx].mean()),
        "distance_weight_mean_test": float(window_data.distance_weights[test_idx].mean()),
        "distance_weight_max_train": float(window_data.distance_weights[train_idx].max()),
        "distance_weight_max_test": float(window_data.distance_weights[test_idx].max()),
        "train_samples": int(len(train_idx)),
        "test_samples": int(len(test_idx)),
        "train_fly_rate": float(window_data.labels[train_idx].mean()),
        "test_fly_rate": float(window_data.labels[test_idx].mean()),
...
def evaluate_triline(
    ...
    with torch.no_grad():
        for features, bird_ids, labels, log_distance_targets, direction_targets, distance_weights in loader:
            features = features.to(device)
            bird_ids = bird_ids.to(device)
            labels = labels.to(device)
            log_distance_targets = log_distance_targets.to(device)
    ...
    for setup_name in setup_names:
        ...
        for path_id in test_path_ids:
            ...
            lstm_lat, lstm_lon, lstm_fly_prob = rollout_triline_model(
                ...
                rollout_steps,
                device,
                fly_probability_threshold=float(lstm_row.get("fly_threshold", 0.5) or 0.5),
                context_days=context_days,
            )
            transformer_lat, transformer_lon, transformer_fly_prob = rollout_triline_model(
                ...
                rollout_steps,
                device,
                fly_probability_threshold=float(transformer_row.get("fly_threshold", 0.5) or 0.5),
                context_days=context_days,
            )
...
<html lang="en">
  <head>
    ...
    <title>DistanceWeightTune Rollout Explorer</title>
    ...
    <main class="app-shell">
      <aside class="sidebar">
        <div class="title-block">
          <p class="eyebrow">DistanceWeightTune</p>
...
def write_analysis(
    ...
    if not rows:
        (output_dir / "analysis.md").write_text("# DistanceWeightTune\n\nNo rows produced.\n", encoding="utf-8")
        return
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output-dir", default="DistanceWeightTune")
...
def main() -> None:
    ...
    write_json(
        output_dir / "distance_weight_config.json",
        {
            ...
            "weighted_losses": ["distance_log_mse", "direction_mse_for_moving_samples"],
            "unweighted_losses": ["fly_bce_with_existing_pos_weight"],
            "bins": DISTANCE_WEIGHT_BINS,
    ...
    write_json(
        output_dir / "run_config.json",
        {
            ...
            "distance_weighted_training": True,
            "distance_weight_scope": "triline models only; distance and direction losses",
            "distance_weight_bins": DISTANCE_WEIGHT_BINS,
            "baseline_comparison_dir": str(WEATHER_BASELINE_OUTPUT_DIR),
            "device": str(device),
            "weather_data": True,
