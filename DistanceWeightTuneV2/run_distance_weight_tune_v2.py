...
WEATHER_BASELINE_OUTPUT_DIR = ROOT / "Path_Experiment_WithWeather"
DISTANCE_WEIGHT_V1_OUTPUT_DIR = ROOT / "DistanceWeightTune"
...
WEIGHT_CONFIGS = {
    "mild_bins": {
        "bins": [1.0, 1.25, 1.75, 2.5],
        "direction_weighted": True,
    },
    "medium_bins": {
        "bins": [1.0, 1.5, 2.5, 4.0],
        "direction_weighted": True,
    },
    "distance_only": {
        "bins": [1.0, 1.5, 2.5, 4.0],
        "direction_weighted": False,
    },
    "current_bins": {
        "bins": [1.0, 2.0, 4.0, 8.0],
        "direction_weighted": True,
    },
}
MOVEMENT_CLASS_NAMES = ["stationary", "short", "medium", "large"]
BALANCED_MEAN_ERROR_TOLERANCE_KM = 0.5
ROLLOUT_THRESHOLD_OFFSETS = [-0.10, -0.05, 0.0, 0.05, 0.10]
...
class WindowData:
    features: np.ndarray
    labels: np.ndarray
    movement_class_targets: np.ndarray
...
class TrilineDataset(Dataset):
    def __init__(
        ...
        self.movement_class_targets = torch.as_tensor(movement_class_targets, dtype=torch.long)
        ...
        self.distance_weights = torch.as_tensor(distance_weights, dtype=torch.float32)
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, ...]:
        return (
            ...
            self.bird_ids[idx],
            self.labels[idx],
            self.movement_class_targets[idx],
            self.log_distance_targets[idx],
            self.direction_targets[idx],
            self.distance_weights[idx],
        )
...
class TrilineLSTM(nn.Module):
    def __init__(
        ...
        dropout: float = 0.1,
        bird_embedding_dim: int = 2,
        use_movement_categories: bool = False,
    ) -> None:
        super().__init__()
        self.use_movement_categories = use_movement_categories
        ...
        self.movement_head = triline_head(hidden_dim, 4, dropout)
        distance_input_dim = hidden_dim + 4 if self.use_movement_categories else hidden_dim
        self.distance_head = triline_head(distance_input_dim, 1, dropout)
    ...
    def forward(self, features: torch.Tensor, bird_ids: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        distance_input = pooled
        if self.use_movement_categories:
            distance_input = torch.cat([pooled, torch.softmax(movement_logits, dim=1)], dim=1)
        return {
            "fly_logit": self.fly_head(pooled).squeeze(-1),
            "movement_logits": movement_logits,
            "log_distance": self.distance_head(distance_input).squeeze(-1),
...
class TrilineLinearAR(nn.Module):
    ...
    def forward(self, features: torch.Tensor, bird_ids: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        movement_logits = self.movement_head(rep)
        distance_input = rep
        if self.use_movement_categories:
            distance_input = torch.cat([rep, torch.softmax(movement_logits, dim=1)], dim=1)
        return {
...
def movement_class_for_step(step_km: float) -> int:
    if step_km <= 5.0:
        return 0
    if step_km <= 30.0:
        return 1
...
def distance_weight_for_step(step_km: float, bins: list[float] | None = None) -> float:
    weights = bins or WEIGHT_CONFIGS["current_bins"]["bins"]
    if step_km <= 5.0:
...
def distance_weights_for_steps(step_km: np.ndarray, bins: list[float]) -> np.ndarray:
    return np.asarray([distance_weight_for_step(float(step), bins) for step in step_km], dtype=np.float32)
...
def build_windows(df: pd.DataFrame, k: int, fly_threshold_km: float) -> WindowData:
    ...
    labels: list[float] = []
    movement_class_targets: list[int] = []
    ...
    return WindowData(
        ...
        movement_class_targets=np.asarray(movement_class_targets, dtype=np.int64),
...
def gps_metrics(
    ...
    if len(errors_km) == 0:
        return {
            ...
            "migration100_error_mean_km": None,
        }
    ...
    migration100_mask = target_step_km > 100.0
    return {
        ...
        "migration100_error_mean_km": subset_mean(migration100_mask),
    }
...
def best_fly_threshold(labels: np.ndarray, scores: np.ndarray) -> tuple[float, dict[str, object], list[dict[str, object]]]:
...
def threshold_sweep(labels: np.ndarray, scores: np.ndarray, output_csv: Path) -> tuple[float, dict[str, object]]:
    best_threshold, best_metrics, rows = best_fly_threshold(labels, scores)
    pd.DataFrame(rows).to_csv(output_csv, index=False)
    return best_threshold, best_metrics


def triline_gps_eval(
    ...
    gated_errors = haversine_km(window_data.target_lat[test_idx], window_data.target_lon[test_idx], gated_lat, gated_lon)
    gated_metrics = {
        f"gated_{key}": value
        for key, value in gps_metrics(gated_errors, window_data.target_step_km[test_idx], fly_threshold_km).items()
    }
    return primary_metrics, gated_metrics


def weather_baseline_metrics(setup_name: str, model_name: str) -> dict[str, float] | None:
    path = WEATHER_BASELINE_OUTPUT_DIR / "comparison_summary.csv"
    if not path.exists():
        return None
    df = pd.read_csv(path)
    rows = df[(df["setup"].astype(str) == setup_name) & (df["model"].astype(str) == model_name)]
    if rows.empty:
        return None
    row = rows.iloc[0]
    return {
        "mean_error_km": float(row["mean_error_km"]),
        "migration50_error_mean_km": float(row["migration50_error_mean_km"]),
        "stationary_error_mean_km": float(row["stationary_error_mean_km"]),
    }


def balanced_checkpoint_score(
    primary_metrics: dict[str, float | None],
    gated_metrics: dict[str, float | None],
    baseline_metrics: dict[str, float] | None,
) -> tuple[bool, float, str]:
    if baseline_metrics is None:
        return False, float("inf"), "missing_weather_baseline"
    mean_error = primary_metrics.get("mean_error_km")
    migration50 = primary_metrics.get("migration50_error_mean_km")
    gated_mean = gated_metrics.get("gated_mean_error_km")
    if mean_error is None or migration50 is None or gated_mean is None:
        return False, float("inf"), "missing_epoch_metric"
    if float(migration50) >= baseline_metrics["migration50_error_mean_km"]:
        return False, float("inf"), "migration50_not_improved"
    if float(mean_error) > baseline_metrics["mean_error_km"] + BALANCED_MEAN_ERROR_TOLERANCE_KM:
        return False, float("inf"), "mean_error_too_high"
    return True, float(gated_mean), "accepted"
...
def multitask_loss(
    ...
    labels: torch.Tensor,
    movement_class_targets: torch.Tensor,
    log_distance_targets: torch.Tensor,
    direction_targets: torch.Tensor,
    distance_weights: torch.Tensor,
    fly_loss_fn: nn.Module,
    movement_loss_fn: nn.Module,
    direction_weighted: bool,
) -> tuple[torch.Tensor, dict[str, float]]:
    fly_loss = fly_loss_fn(outputs["fly_logit"], labels)
    movement_loss = movement_loss_fn(outputs["movement_logits"], movement_class_targets)
    sample_weights = distance_weights.clamp_min(1.0)
    distance_error = (outputs["log_distance"] - log_distance_targets).pow(2)
    distance_loss = (distance_error * sample_weights).sum() / sample_weights.sum().clamp_min(1.0)
    unweighted_distance_loss = distance_error.mean()
    moving_weight = labels.view(-1, 1)
    direction_sample_weights = sample_weights if direction_weighted else torch.ones_like(sample_weights)
    moving_sample_weights = moving_weight * direction_sample_weights.view(-1, 1)
    direction_error = (outputs["direction"] - direction_targets).pow(2)
    direction_loss = (
        (direction_error * moving_sample_weights).sum()
        / moving_sample_weights.sum().clamp_min(1.0)
    )
    unweighted_direction_loss = (
        (direction_error * moving_weight).sum() / moving_weight.sum().clamp_min(1.0)
    )
    total = fly_loss + distance_loss + 0.5 * direction_loss + 0.25 * movement_loss
    unweighted_total = fly_loss + unweighted_distance_loss + 0.5 * unweighted_direction_loss + 0.25 * movement_loss
    return total, {
        "fly_loss": float(fly_loss.detach().cpu().item()),
        "movement_class_loss": float(movement_loss.detach().cpu().item()),
        "distance_loss": float(distance_loss.detach().cpu().item()),
        "direction_loss": float(direction_loss.detach().cpu().item()),
        "unweighted_distance_loss": float(unweighted_distance_loss.detach().cpu().item()),
        "unweighted_direction_loss": float(unweighted_direction_loss.detach().cpu().item()),
        "unweighted_total_loss": float(unweighted_total.detach().cpu().item()),
        "mean_distance_weight": float(sample_weights.detach().mean().cpu().item()),
    }
...
def train_triline_model(
    *,
    ...
    weight_config_name = str(model_spec.get("weight_config", "current_bins"))
    weight_config = WEIGHT_CONFIGS[weight_config_name]
    weight_bins = [float(value) for value in weight_config["bins"]]
    direction_weighted = bool(weight_config["direction_weighted"])
    spec_distance_weights = distance_weights_for_steps(window_data.target_step_km, weight_bins)

    dataset = TrilineDataset(
        normalized_features,
        window_data.bird_ids,
        window_data.labels,
        window_data.movement_class_targets,
        window_data.log_distance_targets,
        window_data.direction_targets,
        spec_distance_weights,
    )
    train_loader = DataLoader(
        Subset(dataset, train_idx.tolist()),
        batch_size=128,
    ...
    if model_kind == "triline_transformer":
        model = TrilineTransformer(
            ...
            n_layers=int(model_spec.get("n_layers", 2)),
            use_bird_id=True,
            use_movement_categories=bool(model_spec.get("use_movement_categories", True)),
        )
    ...
    movement_loss_fn = nn.CrossEntropyLoss()
    optimizer = torch.optim.AdamW(model.parameters(), lr=1e-3, weight_decay=1e-4)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer, mode="min", factor=0.5, patience=4
    )

    baseline_metrics = weather_baseline_metrics(setup_name, str(model_spec.get("baseline_model", model_spec["name"])))
    best_loss = float("inf")
    best_balanced_score = float("inf")
    best_selection_reason = "weighted_validation_loss_fallback"
    best_epoch = 0
    epochs_without_improvement = 0
    log_rows: list[dict[str, object]] = []
    checkpoint_path = model_dir / "best_model.pt"
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        component_sums = {
            "fly_loss": 0.0,
            "movement_class_loss": 0.0,
            "distance_loss": 0.0,
            "direction_loss": 0.0,
            "unweighted_distance_loss": 0.0,
            "unweighted_direction_loss": 0.0,
            "unweighted_total_loss": 0.0,
            "mean_distance_weight": 0.0,
        }
        for features, bird_ids, labels, movement_class_targets, log_distance_targets, direction_targets, distance_weights in train_loader:
            features = features.to(device)
            bird_ids = bird_ids.to(device)
            labels = labels.to(device)
            movement_class_targets = movement_class_targets.to(device)
            log_distance_targets = log_distance_targets.to(device)
            direction_targets = direction_targets.to(device)
            distance_weights = distance_weights.to(device)
            optimizer.zero_grad(set_to_none=True)
            outputs = model(features, bird_ids)
            loss, components = multitask_loss(
                outputs,
                labels,
                movement_class_targets,
        ...
        test_loss, test_unweighted_loss, labels_np, probabilities, pred_distances_epoch, pred_directions_epoch = evaluate_triline(
            model, test_loader, fly_loss_fn, movement_loss_fn, direction_weighted, device
        )
        fixed_fly_metrics = fly_metrics(labels_np, probabilities, 0.5)
        epoch_threshold, _, _ = best_fly_threshold(labels_np, probabilities)
        epoch_primary_metrics, epoch_gated_metrics = triline_gps_eval(
            window_data,
            test_idx,
            pred_distances_epoch,
            pred_directions_epoch,
            probabilities,
            epoch_threshold,
            fly_threshold_km,
        )
        balanced_accepted, balanced_score, balanced_reason = balanced_checkpoint_score(
            epoch_primary_metrics,
            epoch_gated_metrics,
            baseline_metrics,
        )
        scheduler.step(test_loss)
        log_rows.append(
            {
                ...
                "train_movement_class_loss": component_sums["movement_class_loss"] / max(train_count, 1),
                ...
                "train_unweighted_distance_loss": component_sums["unweighted_distance_loss"] / max(train_count, 1),
                "train_unweighted_direction_loss": component_sums["unweighted_direction_loss"] / max(train_count, 1),
                "train_unweighted_total_loss": component_sums["unweighted_total_loss"] / max(train_count, 1),
                "train_mean_distance_weight": component_sums["mean_distance_weight"] / max(train_count, 1),
                "test_loss": test_loss,
                "test_unweighted_loss": test_unweighted_loss,
                "epoch_fly_threshold": epoch_threshold,
                "epoch_mean_error_km": epoch_primary_metrics.get("mean_error_km"),
                "epoch_migration50_error_mean_km": epoch_primary_metrics.get("migration50_error_mean_km"),
                "epoch_stationary_error_mean_km": epoch_primary_metrics.get("stationary_error_mean_km"),
                "epoch_gated_mean_error_km": epoch_gated_metrics.get("gated_mean_error_km"),
                "balanced_checkpoint_accepted": balanced_accepted,
                "balanced_checkpoint_score": balanced_score,
                "balanced_checkpoint_reason": balanced_reason,
        ...
        fallback_improved = test_loss < best_loss - 1e-5
        balanced_improved = balanced_accepted and balanced_score < best_balanced_score - 1e-5
        if balanced_improved or (best_selection_reason == "weighted_validation_loss_fallback" and fallback_improved):
            if balanced_improved:
                best_balanced_score = balanced_score
                best_selection_reason = "balanced_checkpoint_score"
            ...
            torch.save(
                {
                    ...
                    "distance_weight_config": weight_config_name,
                    "distance_weight_bins": weight_bins,
                    "distance_weight_direction_weighted": direction_weighted,
                    "distance_weight_scope": (
                        "distance_and_direction_losses" if direction_weighted else "distance_loss_only"
                    ),
                    "movement_categories": MOVEMENT_CLASS_NAMES,
                    "baseline_model": str(model_spec.get("baseline_model", model_spec["name"])),
    ...
    checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
    model.load_state_dict(checkpoint["model_state_dict"])
    final_loss, final_unweighted_loss, labels_np, probabilities, pred_distances, pred_directions = evaluate_triline(
        model, test_loader, fly_loss_fn, movement_loss_fn, direction_weighted, device
    )
    ...
    primary_metrics, gated_metrics = triline_gps_eval(
        window_data,
        test_idx,
        pred_distances,
        pred_directions,
    ...
    metrics_payload: dict[str, object] = {
        "setup_fly_threshold_km": fly_threshold_km,
        "model": model_spec["name"],
        "baseline_model": model_spec.get("baseline_model", model_spec["name"]),
        ...
        "best_test_loss": best_loss,
        "selection_reason": best_selection_reason,
        "best_balanced_score": None if best_balanced_score == float("inf") else best_balanced_score,
        "weather_baseline_mean_error_km": None if baseline_metrics is None else baseline_metrics["mean_error_km"],
        "weather_baseline_migration50_error_mean_km": None if baseline_metrics is None else baseline_metrics["migration50_error_mean_km"],
        "final_test_loss": final_loss,
        "final_unweighted_test_loss": final_unweighted_loss,
        "raw_pos_weight": raw_pos_weight,
        "pos_weight": pos_weight_value,
        "distance_weighted_training": True,
        "distance_weight_config": weight_config_name,
        "distance_weight_scope": "distance_and_direction_losses" if direction_weighted else "distance_loss_only",
        "distance_weight_bins": weight_bins,
        "distance_weight_mean_train": float(spec_distance_weights[train_idx].mean()),
        "distance_weight_mean_test": float(spec_distance_weights[test_idx].mean()),
        "distance_weight_max_train": float(spec_distance_weights[train_idx].max()),
        "distance_weight_max_test": float(spec_distance_weights[test_idx].max()),
        "movement_categories": MOVEMENT_CLASS_NAMES,
        "train_samples": int(len(train_idx)),
        "test_samples": int(len(test_idx)),
        "train_fly_rate": float(window_data.labels[train_idx].mean()),
...
def evaluate_triline(
    ...
    with torch.no_grad():
        for features, bird_ids, labels, movement_class_targets, log_distance_targets, direction_targets, distance_weights in loader:
            features = features.to(device)
            bird_ids = bird_ids.to(device)
            labels = labels.to(device)
            movement_class_targets = movement_class_targets.to(device)
            ...
            total_unweighted_loss += float(components["unweighted_total_loss"]) * int(labels.shape[0])
    ...
    return (
        ...
        total_unweighted_loss / max(total_count, 1),
...
def full_model_specs() -> list[dict[str, object]]:
    specs: list[dict[str, object]] = [
        ...
        {"name": "direct_transformer_2l_k14", "kind": "direct_transformer", "k": 14, "n_layers": 2},
        {"name": "direct_mlp_last_day_k1", "kind": "direct_mlp_last", "k": 1},
    ]
    base_specs = [
        {"baseline_model": "triline_lstm_2l_k7", "kind": "triline_lstm", "k": 7, "n_layers": 2},
        {"baseline_model": "triline_transformer_2l_k7", "kind": "triline_transformer", "k": 7, "n_layers": 2},
        {"baseline_model": "triline_transformer_4l_k7", "kind": "triline_transformer", "k": 7, "n_layers": 4},
        {"baseline_model": "triline_lstm_2l_k14", "kind": "triline_lstm", "k": 14, "n_layers": 2},
    ]
    for base in base_specs:
        for weight_config in WEIGHT_CONFIGS:
            specs.append(
                {
                    "name": f"{base['baseline_model']}__{weight_config}",
                    "baseline_model": base["baseline_model"],
                    "kind": base["kind"],
                    "k": base["k"],
                    "n_layers": base["n_layers"],
                    "weight_config": weight_config,
                    "use_movement_categories": True,
                }
            )
    return specs
...
def smoke_model_specs() -> list[dict[str, object]]:
    return [
        {"name": "direct_transformer_2l_k7", "kind": "direct_transformer", "k": 7, "n_layers": 2},
        {
            "name": "triline_lstm_2l_k7__mild_bins",
            "baseline_model": "triline_lstm_2l_k7",
            "kind": "triline_lstm",
            "k": 7,
            "n_layers": 2,
            "weight_config": "mild_bins",
            "use_movement_categories": True,
        },
        {
            "name": "triline_transformer_2l_k7__distance_only",
            "baseline_model": "triline_transformer_2l_k7",
            "kind": "triline_transformer",
            "k": 7,
            "n_layers": 2,
            "weight_config": "distance_only",
            "use_movement_categories": True,
        },
    ]
...
def rollout_triline_model(
    ...
    rollout_steps: int,
    device: torch.device,
    fly_probability_threshold: float,
    context_days: int = ROLLOUT_CONTEXT_K,
    ...
    with torch.no_grad():
        for step in range(rollout_steps):
            ...
            if fly_prob < fly_probability_threshold:
                distance = 0.0
...
def select_rollout_path(
    ...
    for path_id, path_df in featured.groupby(GROUP_COLUMN, sort=False):
        path_df = path_df.sort_values("date").reset_index(drop=True)
        dates = pd.to_datetime(path_df["date"], utc=True).dt.normalize()
        ...
        if len(valid_df) >= min_rows and pd.to_datetime(valid_df["date"], utc=True).min() >= test_start:
...
def best_summary_row(
    ...
    if family == "triline" and "selection_reason" in summary.columns:
        accepted = summary[summary["selection_reason"].astype(str) == "balanced_checkpoint_score"].copy()
        if not accepted.empty and "gated_mean_error_km" in accepted.columns:
            row = accepted.loc[pd.to_numeric(accepted["gated_mean_error_km"], errors="coerce").idxmin()]
            return row.to_dict()
    ...
    *,
    ...
    for setup_name in setup_names:
        ...
        triline_lat, triline_lon, triline_fly_prob = rollout_triline_model(
            ...
            actual_rollout_steps,
            device,
            fly_probability_threshold=float(triline_row.get("fly_threshold", 0.5) or 0.5),
        )
...
def threshold_candidates(base_threshold: float) -> list[float]:
    values = [min(max(float(base_threshold) + offset, 0.01), 0.99) for offset in ROLLOUT_THRESHOLD_OFFSETS]
    return sorted(set(round(value, 6) for value in values))


def calibrate_rollout_threshold(
    *,
    model: nn.Module,
    checkpoint: dict[str, object],
    ...
    best_mean = float("inf")
    for threshold in threshold_candidates(base_threshold):
        path_means: list[float] = []
        path_finals: list[float] = []
        for path_df in path_dfs:
            context_days, _ = observed_context_days_for_path(path_df)
            if context_days >= len(path_df):
                continue
            rollout_steps = len(path_df) - context_days
            lat_values, lon_values, _ = rollout_triline_model(
                model,
                checkpoint,
                path_df,
                rollout_steps,
                device,
                fly_probability_threshold=threshold,
                context_days=context_days,
            )
            mean_error, final_error = rollout_error_summary(path_df, lat_values, lon_values, context_days)
            if mean_error is not None:
                path_means.append(float(mean_error))
            if final_error is not None:
                path_finals.append(float(final_error))
        aggregate_mean = float(np.mean(path_means)) if path_means else float("inf")
        rows.append(
            {
                "setup": setup_name,
                "model": model_label,
                "threshold": float(threshold),
                "paths": int(len(path_means)),
                "mean_rollout_error_km": None if aggregate_mean == float("inf") else aggregate_mean,
                "final_rollout_error_km": float(np.mean(path_finals)) if path_finals else None,
            }
        )
        if aggregate_mean < best_mean:
            best_mean = aggregate_mean
            best_threshold = float(threshold)
...
def run_all_test_rollouts(
    *,
    ...
    threshold_calibration_rows: list[dict[str, object]] = []
    ...
    for setup_name in setup_names:
        ...
        calibration_path_dfs: list[pd.DataFrame] = []
        for path_id in test_path_ids:
            ...
            context_days, _ = observed_context_days_for_path(candidate_df)
            if context_days < len(candidate_df):
                calibration_path_dfs.append(candidate_df)
        lstm_rollout_threshold, lstm_calibration = calibrate_rollout_threshold(
            model=lstm_model,
            checkpoint=lstm_checkpoint,
            path_dfs=calibration_path_dfs,
            base_threshold=float(lstm_row.get("fly_threshold", 0.5) or 0.5),
            device=device,
            setup_name=setup_name,
            model_label=str(lstm_row["model"]),
        )
        transformer_rollout_threshold, transformer_calibration = calibrate_rollout_threshold(
            model=transformer_model,
            checkpoint=transformer_checkpoint,
            path_dfs=calibration_path_dfs,
            base_threshold=float(transformer_row.get("fly_threshold", 0.5) or 0.5),
            device=device,
            setup_name=setup_name,
            model_label=str(transformer_row["model"]),
        )
        threshold_calibration_rows.extend(lstm_calibration)
        threshold_calibration_rows.extend(transformer_calibration)
        ...
        for path_id in test_path_ids:
            ...
            lstm_lat, lstm_lon, lstm_fly_prob = rollout_triline_model(
                ...
                rollout_steps,
                device,
                fly_probability_threshold=lstm_rollout_threshold,
                context_days=context_days,
            )
            transformer_lat, transformer_lon, transformer_fly_prob = rollout_triline_model(
                ...
                rollout_steps,
                device,
                fly_probability_threshold=transformer_rollout_threshold,
                context_days=context_days,
            )
            ...
            app_rows.extend(
                build_rollout_rows_for_path(
                    ...
                    lstm_model_name=f"{lstm_row['model']} (distance weighted, fly-gated)",
                    transformer_model_name=f"{transformer_row['model']} (distance weighted, fly-gated)",
                )
            )
            ...
            summary_rows.append(
                {
                    ...
                    "lstm_model": f"{lstm_row['model']} (distance weighted, fly-gated)",
                    "lstm_fly_threshold": float(lstm_row.get("fly_threshold", 0.5) or 0.5),
                    "lstm_rollout_threshold": float(lstm_rollout_threshold),
                    "lstm_mean_rollout_error_km": lstm_mean,
                    "lstm_final_rollout_error_km": lstm_final,
                    "transformer_model": f"{transformer_row['model']} (distance weighted, fly-gated)",
                    "transformer_fly_threshold": float(transformer_row.get("fly_threshold", 0.5) or 0.5),
                    "transformer_rollout_threshold": float(transformer_rollout_threshold),
    ...
    write_csv(all_output_dir / "rollout_threshold_calibration.csv", threshold_calibration_rows)
    write_json(
...
<html lang="en">
  <head>
    ...
    <title>DistanceWeightTune V2 Rollout Explorer</title>
    ...
    <main class="app-shell">
      <aside class="sidebar">
        <div class="title-block">
          <p class="eyebrow">DistanceWeightTune V2</p>
...
.nofly-dot { fill: #eef2ef; opacity: 0.72; stroke-dasharray: 2 2; }
...
function flyThresholdFor(row, modelKey) {
  ...
  const threshold = Number(summary[`${modelKey}_rollout_threshold`] ?? summary[`${modelKey}_fly_threshold`]);
...
COMPARISON_METRICS = [
    ...
    "gated_migration100_error_mean_km",
    "gated_stationary_error_mean_km",
    "gated_fly_error_mean_km",
]


def create_distance_weight_vs_weather_comparison(
    output_dir: Path,
    weighted_rows: list[dict[str, object]],
) -> list[dict[str, object]]:
    baseline_csv = WEATHER_BASELINE_OUTPUT_DIR / "comparison_summary.csv"
    if not baseline_csv.exists() or not weighted_rows:
        return []

    weighted_df = pd.DataFrame(weighted_rows)
    baseline_df = pd.read_csv(baseline_csv)
    weighted_df["baseline_model_join"] = weighted_df.get("baseline_model", weighted_df.get("model")).fillna(weighted_df["model"])
    baseline_df["baseline_model_join"] = baseline_df["model"].astype(str)
    join_keys = ["setup", "model_family", "kind", "k", "baseline_model_join"]
    for key in join_keys:
        if key not in weighted_df.columns or key not in baseline_df.columns:
            return []
    ...
    for _, row in merged.iterrows():
        payload: dict[str, object] = {
            ...
            "model": row.get("model_weighted", row.get("model", "")),
            "baseline_model": row["baseline_model_join"],
            "kind": row["kind"],
            "k": row["k"],
        }
        for metric in COMPARISON_METRICS:
            weighted_key = f"{metric}_weighted"
            weather_key = f"{metric}_weather"
            if weighted_key not in row.index or weather_key not in row.index:
                continue
            weighted_value = pd.to_numeric(pd.Series([row[weighted_key]]), errors="coerce").iloc[0]
            weather_value = pd.to_numeric(pd.Series([row[weather_key]]), errors="coerce").iloc[0]
            if pd.isna(weighted_value) or pd.isna(weather_value):
                continue
            payload[f"{metric}_weighted"] = float(weighted_value)
            payload[f"{metric}_weather"] = float(weather_value)
            payload[f"{metric}_delta_weighted_minus_weather"] = float(weighted_value - weather_value)
            payload[f"{metric}_improved"] = bool(weighted_value < weather_value)
        rows.append(payload)

    write_csv(output_dir / "distance_weight_vs_weather_comparison.csv", rows)
    return rows


def create_rollout_distance_weight_vs_weather_comparison(output_dir: Path) -> list[dict[str, object]]:
    weighted_csv = output_dir / "rollout_all" / "rollout_all_summary.csv"
    baseline_csv = WEATHER_BASELINE_OUTPUT_DIR / "rollout_all" / "rollout_all_summary.csv"
    if not weighted_csv.exists() or not baseline_csv.exists():
        return []

    weighted_df = pd.read_csv(weighted_csv)
    baseline_df = pd.read_csv(baseline_csv)
    ...
    for _, row in merged.iterrows():
        ...
        for metric in metric_columns:
            ...
            payload[f"{metric}_improved"] = bool(weighted_value < weather_value)
        rows.append(payload)
    write_csv(output_dir / "rollout_distance_weight_vs_weather_comparison.csv", rows)
    return rows


def create_v2_vs_v1_comparison(output_dir: Path, weighted_rows: list[dict[str, object]]) -> list[dict[str, object]]:
    v1_csv = DISTANCE_WEIGHT_V1_OUTPUT_DIR / "comparison_summary.csv"
    if not v1_csv.exists() or not weighted_rows:
        return []
    weighted_df = pd.DataFrame(weighted_rows)
    v1_df = pd.read_csv(v1_csv)
    weighted_df["baseline_model_join"] = weighted_df.get("baseline_model", weighted_df.get("model")).fillna(weighted_df["model"])
    v1_df["baseline_model_join"] = v1_df["model"].astype(str)
    join_keys = ["setup", "model_family", "kind", "k", "baseline_model_join"]
    for key in join_keys:
        if key not in weighted_df.columns or key not in v1_df.columns:
            return []
    merged = weighted_df.merge(v1_df, on=join_keys, how="inner", suffixes=("_v2", "_v1"))
    rows: list[dict[str, object]] = []
    for _, row in merged.iterrows():
        payload: dict[str, object] = {
            "setup": row["setup"],
            "model_family": row["model_family"],
            "model": row.get("model_v2", ""),
            "v1_model": row["baseline_model_join"],
            "kind": row["kind"],
            "k": row["k"],
        }
        for metric in COMPARISON_METRICS:
            v2_key = f"{metric}_v2"
            v1_key = f"{metric}_v1"
            if v2_key not in row.index or v1_key not in row.index:
                continue
            v2_value = pd.to_numeric(pd.Series([row[v2_key]]), errors="coerce").iloc[0]
            v1_value = pd.to_numeric(pd.Series([row[v1_key]]), errors="coerce").iloc[0]
            if pd.isna(v2_value) or pd.isna(v1_value):
                continue
            payload[f"{metric}_v2"] = float(v2_value)
            payload[f"{metric}_v1"] = float(v1_value)
            payload[f"{metric}_delta_v2_minus_v1"] = float(v2_value - v1_value)
            payload[f"{metric}_improved"] = bool(v2_value < v1_value)
        rows.append(payload)
    write_csv(output_dir / "distance_weight_v2_vs_v1_comparison.csv", rows)
    return rows


def write_training_log_summary(output_dir: Path) -> list[dict[str, object]]:
    rows: list[dict[str, object]] = []
    for log_path in sorted(output_dir.glob("setup_*/k_*/*/training_log.csv")):
        try:
        ...
        k_dir = model_dir.parent
        setup_dir = k_dir.parent
        row: dict[str, object] = {
            "setup": setup_dir.name,
            "k": int(k_dir.name.replace("k_", "")) if k_dir.name.startswith("k_") else "",
        ...
        for column in [
            "train_loss",
            "test_loss",
            "test_unweighted_loss",
            "epoch_mean_error_km",
            "epoch_migration50_error_mean_km",
            "epoch_gated_mean_error_km",
            "train_movement_class_loss",
            "train_distance_loss",
            "train_unweighted_distance_loss",
            "train_direction_loss",
            "train_unweighted_direction_loss",
            "train_mean_distance_weight",
            "test_delta_mse_norm",
        ]:
            if column in df.columns:
                numeric = pd.to_numeric(df[column], errors="coerce").dropna()
                if len(numeric):
                    row[f"last_{column}"] = float(numeric.iloc[-1])
                    row[f"min_{column}"] = float(numeric.min())
        rows.append(row)
    write_csv(output_dir / "training_log_summary.csv", rows)
    return rows
...
def write_analysis(
    ...
    if not rows:
        (output_dir / "analysis.md").write_text("# DistanceWeightTune V2\n\nNo rows produced.\n", encoding="utf-8")
        return
    ...
    if comparison_rows:
        ...
        display_comparison = comparison_df.dropna(
            subset=["mean_error_km_weighted", "mean_error_km_weather"]
        ).sort_values(["setup", "mean_error_km_weighted"])
        for _, row in display_comparison.iterrows():
            delta = float(row["mean_error_km_delta_weighted_minus_weather"])
            migration_delta = row.get("migration50_error_mean_km_delta_weighted_minus_weather", "")
            migration_text = "" if migration_delta == "" or pd.isna(migration_delta) else f"{float(migration_delta):.4f}"
            improved = "yes" if bool(row["mean_error_km_improved"]) else "no"
            lines.append(
                f"| {row['setup']} | {row['model']} | {row['model_family']} | {int(row['k'])} | "
                f"{float(row['mean_error_km_weighted']):.4f} | {float(row['mean_error_km_weather']):.4f} | "
                f"{delta:.4f} | {migration_text} | {improved} |"
            )

        triline_comparison = comparison_df[comparison_df["model_family"] == "triline"].copy()
        if not triline_comparison.empty and "migration50_error_mean_km_delta_weighted_minus_weather" in triline_comparison:
            improved_migration = pd.to_numeric(
                triline_comparison["migration50_error_mean_km_delta_weighted_minus_weather"],
                errors="coerce",
            ) < 0
            lines.extend(
                [
                    "",
                    "Distance-weighted triline rows improving migration50 error: "
                    f"{int(improved_migration.sum())}/{int(improved_migration.notna().sum())}.",
                ]
            )
    ...
    for _, row in display.iterrows():
        ...
        lines.append(
            ...
            f"{fmt_cell(row.get('migration50_error_mean_km'))} | {fmt_cell(row.get('migration100_error_mean_km'))} |"
        )

    training_summary_path = output_dir / "training_log_summary.csv"
    if training_summary_path.exists():
        ...
        lines.extend(
            [
                ...
                "## Training Logs",
                "",
                f"- Summarized {len(training_df)} training logs in `training_log_summary.csv`.",
            ]
        )
        weighted_cols = [
            "last_train_distance_loss",
            "last_train_unweighted_distance_loss",
            "last_test_loss",
            "last_test_unweighted_loss",
        ]
        present_cols = [column for column in weighted_cols if column in training_df.columns]
        if present_cols:
            lines.append("- Weighted and unweighted triline loss columns are included for auditability.")

    rollout_comparison_path = output_dir / "rollout_distance_weight_vs_weather_comparison.csv"
    rollout_index_path = output_dir / "rollout_all" / "rollout_all_index.json"
    if rollout_index_path.exists():
        with rollout_index_path.open("r", encoding="utf-8") as handle:
            rollout_index = json.load(handle)
        lines.extend(
            [
                "",
                "## Rollout Visualization",
                "",
                f"- Interactive rollout explorer: `rollout_all/index.html`.",
                f"- Rollout paths: {rollout_index.get('paths', 0)}; skipped paths: {rollout_index.get('skipped_paths', 0)}.",
            ]
        )
    if rollout_comparison_path.exists():
        rollout_df = pd.read_csv(rollout_comparison_path)
        lines.append(f"- Rollout comparison rows against weather baseline: {len(rollout_df)}.")
...
def refresh_analysis_from_disk(output_dir: Path) -> None:
    comparison_path = output_dir / "comparison_summary.csv"
    if not comparison_path.exists():
        return
    rows = pd.read_csv(comparison_path).to_dict("records")
    write_training_log_summary(output_dir)
    comparison_rows = create_distance_weight_vs_weather_comparison(output_dir, rows)
    create_v2_vs_v1_comparison(output_dir, rows)
    write_analysis(output_dir, rows, comparison_rows)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Distance-weighted weather-enabled southbound path experiment")
    parser.add_argument("--output-dir", default="DistanceWeightTuneV2")
...
def main() -> None:
    ...
    raw_df = pd.read_csv(SOUTHBOUND_CSV)
    weather_df = load_weather_data()
    df, weather_match_summary = add_weather_to_paths(raw_df, weather_df)
    matched_csv = output_dir / "dataset2_southbound_paths_with_weather.csv"
    df.to_csv(matched_csv, index=False)
    write_json(output_dir / "weather_match_summary.json", weather_match_summary)
    write_json(
        output_dir / "distance_weight_config.json",
        {
            "enabled": True,
            "scope": "triline models only",
            "weighted_losses": ["distance_log_mse", "optional_direction_mse_for_moving_samples"],
            "unweighted_losses": ["fly_bce_with_existing_pos_weight"],
            "weight_configs": WEIGHT_CONFIGS,
            "movement_categories": MOVEMENT_CLASS_NAMES,
            "balanced_mean_error_tolerance_km": BALANCED_MEAN_ERROR_TOLERANCE_KM,
            "rollout_threshold_offsets": ROLLOUT_THRESHOLD_OFFSETS,
            "baseline_comparison_dir": str(WEATHER_BASELINE_OUTPUT_DIR),
            "v1_comparison_dir": str(DISTANCE_WEIGHT_V1_OUTPUT_DIR),
        },
    )
    rollout_setup_names = (
    ...
    if args.rollout_only:
        ...
        create_rollout_distance_weight_vs_weather_comparison(output_dir)
        refresh_analysis_from_disk(output_dir)
        os._exit(0)
    ...
    write_json(
        output_dir / "run_config.json",
        {
            ...
            "distance_weighted_training": True,
            "distance_weight_scope": "triline ablation; distance loss plus optional direction loss",
            "distance_weight_configs": WEIGHT_CONFIGS,
            "movement_categories": MOVEMENT_CLASS_NAMES,
            "checkpoint_selection": {
                "mode": "balanced",
                "require_migration50_improvement": True,
                "max_mean_error_worsening_km": BALANCED_MEAN_ERROR_TOLERANCE_KM,
                "score": "gated_mean_error_km",
            },
            "rollout_threshold_offsets": ROLLOUT_THRESHOLD_OFFSETS,
            "baseline_comparison_dir": str(WEATHER_BASELINE_OUTPUT_DIR),
            "v1_comparison_dir": str(DISTANCE_WEIGHT_V1_OUTPUT_DIR),
    ...
    write_training_log_summary(output_dir)
    comparison_rows = create_distance_weight_vs_weather_comparison(output_dir, all_rows)
    create_v2_vs_v1_comparison(output_dir, all_rows)
