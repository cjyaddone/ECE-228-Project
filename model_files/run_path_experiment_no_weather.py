...
def make_chronological_split(window_data: WindowData, train_fraction: float = 0.8) -> tuple[np.ndarray, np.ndarray]:
    """Split windows by target date: first 80% train, last 20% test."""
    order = np.argsort(window_data.target_dates)
    split_idx = int(len(order) * train_fraction)
    if split_idx <= 0 or split_idx >= len(order):
        raise ValueError(f"Chronological split needs train and test samples, got {len(order)} windows")
...
<html lang="en">
  <head>
    ...
    <title>No-Weather Rollout Explorer</title>
    ...
    <main class="app-shell">
      <aside class="sidebar">
        <div class="title-block">
          <p class="eyebrow">Path Experiment No Weather</p>
...
def write_analysis(output_dir: Path, rows: list[dict[str, object]]) -> None:
    if not rows:
        (output_dir / "analysis.md").write_text("# Path Experiment No Weather\n\nNo rows produced.\n", encoding="utf-8")
        return
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="No-weather southbound path trajectory experiment")
    parser.add_argument("--output-dir", default="Path_Experiment_NoWeather")
...
def main() -> None:
    ...
    write_analysis(output_dir, all_rows)
